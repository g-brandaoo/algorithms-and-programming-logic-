<!-- BANNER -->

<div align="center">
<img src="https://github.com/g-brandaoo/algorithms-and-programming-logic-/blob/main/bannerblue.png" width="100%" alt="banner"/>
</div>

---

# Sistema de Cadastro em Portugol

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=Objetivo+do+projeto" alt="Typing SVG" /></a>

* Este projeto foi desenvolvido para praticar lógica de programação utilizando Portugol no Visualg.

* O sistema permite cadastrar usuários, listar informações, editar dados e remover registros através de um menu interativo simples.

* A ideia principal foi praticar estruturas básicas da programação criando um sistema organizado e funcional.

* Esse projeto ajudou no desenvolvimento do raciocínio lógico e na manipulação de vetores.

---

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=Funcionalidades" alt="Typing SVG" /></a>

* Cadastro de usuários
* Cadastro de nome e idade
* Listagem de usuários
* Edição de usuários
* Remoção de usuários
* Menu interativo
* Encerramento do sistema

---

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=O+que+foi+praticado" alt="Typing SVG" /></a>

## Algoritmos

Foi praticada a construção de algoritmos organizados e estruturados.

---

## Entrada e saída de dados

* Utilização de:
* leia
* escreva
* escreval

---

## Operadores

Uso de:

* operadores matemáticos
* operadores relacionais
* operadores lógicos

---

## Condicionais

Uso de:

* se
* senao
* escolha caso

---

## Estruturas de repetição

Uso de:

* repita
* para

---

## Vetores

Armazenamento de nomes e idades dos usuários.

---

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=Conceitos+usados" alt="Typing SVG" /></a>

## Portugol

Linguagem utilizada para aprendizado de lógica de programação.

---

## Visualg

Programa utilizado para executar algoritmos em Portugol.

---

## Lógica de programação

Utilização de sequência lógica para resolver problemas.

---

## Vetores

Estrutura utilizada para armazenar vários valores em uma única variável.

---

## Condicionais

Estruturas utilizadas para tomada de decisões.

---

## Repetição

Estruturas utilizadas para repetir ações no sistema.

---

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=Prints+do+sistema" alt="Typing SVG" /></a>

## Menu principal

<img src="https://github.com/g-brandaoo/logicprogrammingcourse/blob/main/projetos/cadastro/prints/menu.png" width="100%" alt="banner"/>

---

## Cadastro de usuário

<img src="https://github.com/g-brandaoo/logicprogrammingcourse/blob/main/projetos/cadastro/prints/cadastro.png" width="100%" alt="banner"/>

---

## Listagem de usuários

<img src="https://github.com/g-brandaoo/logicprogrammingcourse/blob/main/projetos/cadastro/prints/listagem.png" width="100%" alt="banner"/>

---

## Edição de usuário

<img src="https://github.com/g-brandaoo/logicprogrammingcourse/blob/main/projetos/cadastro/prints/edicao.png" width="100%" alt="banner"/>

---

## Remoção de usuário

<img src="https://github.com/g-brandaoo/logicprogrammingcourse/blob/main/projetos/cadastro/prints/remocao.png" width="100%" alt="banner"/>

---

<a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.herokuapp.com?font=Montserrat&duration=2000&pause=1000&color=89CFF0&repeat=false&width=435&lines=O+que+voc%C3%AA+aprendeu" alt="Typing SVG" /></a>

Durante esse projeto eu consegui entender melhor como funciona um sistema de cadastro utilizando lógica de programação.

No começo eu tive dificuldade para organizar os vetores e controlar as posições dos usuários cadastrados. Depois de praticar mais, comecei a entender melhor como armazenar dados relacionados ao mesmo usuário sem perder informações.

Também consegui entender melhor como funcionam menus interativos e estruturas condicionais dentro de um sistema maior.

A parte de edição e remoção de usuários ajudou bastante no meu raciocínio lógico, principalmente para validar posições existentes dentro do vetor.

As estruturas de repetição ficaram mais claras para mim durante a listagem de usuários e na navegação do menu principal.

Depois de terminar o sistema, ficou mais fácil entender como vários conceitos básicos podem trabalhar juntos para criar um sistema funcional e organizado.

No futuro quero aprender:

* *Python e outras linguagens*
* *Cibersegurança*
* *Segurança da nuvem*
* *Redes*
* *Linux*
* *Desenvolvimento de sistemas reais*

Esse projeto foi importante porque mostrou na prática como utilizar lógica de programação para construir um sistema simples de cadastro de usuários.
